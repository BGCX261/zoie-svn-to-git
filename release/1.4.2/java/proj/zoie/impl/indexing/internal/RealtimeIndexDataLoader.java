...
public class RealtimeIndexDataLoader<R extends IndexReader, V> extends BatchedIndexDataLoader<R,V>
{
  ...
  public RealtimeIndexDataLoader(DataConsumer<ZoieIndexable> dataLoader, int batchSize,int maxBatchSize,long delay,
                                 Analyzer analyzer,
                                 Similarity similarity,
                                 SearchIndexManager<R> idxMgr,
                                 IndexableInterpreter<V> interpreter,
                                 List<IndexingEventListener> lsnrList)
  {
    ...
    _luceneDataLoader = (DiskLuceneIndexDataLoader<R>)dataLoader;
  }
    ...
    if (events != null)
    {
      ...
      while (iter.hasNext())
      {
        try
        {
          ...
          {
            indexable =
                ZoieIndexableDecorator.decorate(_interpreter.interpret(event.getData()));
          }
