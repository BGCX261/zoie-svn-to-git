...
public class IndexReaderDispenser {
	...
	static
	{
		...
		if (osname!=null && !osname.toLowerCase().contains("windows"))   // it is documented that NIOFSDirectory does not work on windows platforms.
		{
	...
	/**
	      ...
	      throws IOException
	  {
	    if (!luceneDir.exists() || !IndexReader.indexExists(luceneDir))
	      return null;
	    Directory dir=NIOFSDirectory.getDirectory(luceneDir);
