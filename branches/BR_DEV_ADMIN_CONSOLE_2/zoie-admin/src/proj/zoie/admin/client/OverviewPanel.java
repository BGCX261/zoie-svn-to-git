...
public class OverviewPanel extends Composite
{
  ...
  HTMLPanel                    topPanel;
  @UiField
  FlowPanel                    memchartPanel;
  @UiField
  FlowPanel                    cpuchartPanel;
  @UiField
  Label                        serverLabel;
  @UiField
  Label                        platformLabel;
  ...
  private void refreshSysInfo()
  {
    _jmxService.getRuntimeSystemInfo(new AsyncCallback<RuntimeSystemInfo>()
    {
      ...
      public void onSuccess(RuntimeSystemInfo usage)
      {
        ...
        {
          memChart.draw(createMemTable(null), createMemOptions("Error: "
              + th.getMessage()));
        }
      }
    ...
    _jmxService.getZoieSystemInfo(new AsyncCallback<ZoieServerInfo>()
    {
      ...
      public void onSuccess(ZoieServerInfo zsi)
      {
        ...
        for (int i = 0; i < names.length; i++)
        {
          ...
          {
            infoTable.setText(counter, 0, names[i]);
            infoTable.getCellFormatter().addStyleName(counter, 0, "flexTable-colLabel");
            infoTable.setText(counter, 1, values[i]);
            counter++;
          }          
        }
      }
    ...
    _jmxService.getDataProviderInfo(new AsyncCallback<ZoieServerInfo>()
    {
      ...
      public void onSuccess(ZoieServerInfo zsi)
      {
        ...
        for (int i = 0; i < names.length; i++)
        {
          if (writeable[i])
          {
            dataProviderTable.setText(counter, 0, names[i]);
            dataProviderTable.getCellFormatter().addStyleName(counter, 0, "flexTable-colLabel");
            ...
            dataProviderTable.setWidget(counter, 1, tb);
            counter++;
          }
        }
        ...
        buttonFlowPanel.add(buttonstart);
        buttonFlowPanel.add(buttonstop);
        dataProviderTable.getCellFormatter().addStyleName(counter, 0, "flexTable-colLabel-nostyle");
        dataProviderTable.setWidget(counter, 1, buttonFlowPanel);
      }
    });
  }
  ...
  private static AbstractDataTable createMemTable(RuntimeSystemInfo sysInfo)
  {
    ...
    data.setValue(1, 1, sysInfo == null ? 0 : sysInfo.getMaxMemory()
        - sysInfo.getFreeMemory());
    return data;
  }
}
